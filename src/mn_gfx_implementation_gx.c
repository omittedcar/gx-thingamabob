...
/*
useful things to rember:
TVNtsc240Ds: 240p
TVNtsc480Int: 480i
"Aa" at the end means antialiasing
*/
/*
#include <stdlib.h>
#include <string.h>
...
#define DEFAULT_FIFO_SIZE 0x40000
...
void apply_video_mode(GXRModeObj *screenMode) {
...
void mn_gfx_init() {
  VIDEO_Init();
  framebuffer	= MEM_K0_TO_K1(SYS_AllocateFramebuffer(&TVNtsc240Ds));
  VIDEO_Configure(&TVNtsc240Ds);
  ...
  GXColor bg = {0x55, 0x00, 0xff, 0xff};
  GX_SetCopyClear(bg, 0x00ffffff);
  apply_video_mode(&TVNtsc240Ds);
  ...
  GX_SetVtxDesc(GX_VA_CLR0, GX_INDEX8);
	GX_SetVtxAttrFmt(GX_VTXFMT0, GX_VA_POS,	GX_POS_XY,	GX_S16,	4);
  ...
  //this is like setting a fragment shader
...
void mn_gfx_update() {
  Mtx44 the_matrix;
  guMtx44Identity(the_matrix);
  GX_LoadPosMtxImm(the_matrix,	GX_PNMTX0);
  GX_InvVtxCache();
  GX_InvalidateTexAll();
